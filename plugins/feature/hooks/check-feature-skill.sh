...
# transcript에서 feature 스킬 사용 흔적 확인
if [ -n "$transcript_path" ] && [ -f "$transcript_path" ]; then
  if grep -q "test-writing\|code-review" "$transcript_path" 2>/dev/null; then
    echo '{"decision": "approve"}'
    exit 0
  fi
fi

...
```bash
/opt/homebrew/bin/gemini -p "{constructed_prompt_with_@image_reference}" --yolo 2>/dev/null
```

**올바른 예시:**
```bash
/opt/homebrew/bin/gemini -p "이 UI 스크린샷의 레이아웃을 분석해주세요. 한국어로 답변해주세요. @/path/to/image.png" --yolo 2>/dev/null
```

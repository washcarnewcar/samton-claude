...
Claude Code는 inline 이미지를 `~/.claude/image-cache/`에 캐시한다. UserPromptSubmit hook이 이를 `/tmp/gemini_images/`에 자동 복사한다.
osascript 클립보드 방식은 사용하지 않는다 (에러 발생 확률 높음).

```bash
# hook이 미리 복사해 둔 이미지 찾기
SESSION_UUID=$(for f in ~/.claude/sessions/*.json; do
...
IMAGE_PATH=$(ls -t /tmp/gemini_images/$SESSION_UUID/*.png 2>/dev/null | head -1)
...
```bash
/opt/homebrew/bin/gemini -p "{constructed_prompt_with_@image_reference}" --yolo --sandbox false 2>/dev/null
```

**올바른 예시:**
```bash
/opt/homebrew/bin/gemini -p "이 UI 스크린샷의 레이아웃을 분석해주세요. 한국어로 답변해주세요. @/path/to/image.png" --yolo --sandbox false 2>/dev/null
```
